...
import { useGists } from '@/api/queries/gists.queries';
import { usePagination } from '@/api/hooks/usePagination';
import { useSearchStore } from '@/store';
import { Gist } from '@/api/types/gist-api.types';
import GistTable from './table/GistTable';
import GistGrid from './grid/GistGrid';
...
export function GitList() {
  ...
  const { data: response, isLoading, error } = useGists(pagination);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="title">Public Gists</h1>
